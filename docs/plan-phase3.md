...
2. **`pre_model_hook` 도입** (USER_PREFERENCE 추출 + LTM 조회용):
   ```python
   def pre_model_hook(state, config, *, store):
       actor_id = config["configurable"]["actor_id"]
       thread_id = config["configurable"]["thread_id"]
       namespace = (actor_id, thread_id)
       # 최신 HumanMessage를 store에 put (USER_PREFERENCE 추출 대상)
       for msg in reversed(state.get("messages", [])):
           if isinstance(msg, HumanMessage):
               store.put(namespace, str(uuid.uuid4()), {"message": msg})
               # (선택) 선호도 검색해 프롬프트에 주입
               prefs = store.search(("preferences", actor_id), query=msg.content, limit=5)
               # messages에 context로 추가해서 모델이 참고하도록
               break
       return {"llm_input_messages": state["messages"]}
   ```
3. **`post_model_hook` 도입** (SUMMARIZATION 요약 품질 확보용):
   ```python
   def post_model_hook(state, config, *, store):
       actor_id = config["configurable"]["actor_id"]
       thread_id = config["configurable"]["thread_id"]
       namespace = (actor_id, thread_id)
       # AIMessage/ToolMessage도 store에 put해야 SUMMARIZATION이 정상 작동
       for msg in reversed(state.get("messages", [])):
           if isinstance(msg, (AIMessage, ToolMessage)):
               store.put(namespace, str(uuid.uuid4()), {"message": msg})
               break
   ```
4. **`create_agent`에 `store` + 양쪽 hook 연결**
5. **LangGraph 자체의 trim/summary 로직과 AgentCore SUMMARIZATION 이중 실행 방지 검토**

### ⚠️ Hook 선택 기준 — 전략별 필요 데이터 다름

| LTM 전략 | 필요 메시지 | 어디서 저장 |
|---|---|---|
| `USER_PREFERENCE` | HumanMessage만 | `pre_model_hook` |
| `SUMMARIZATION` | HumanMessage + AIMessage + ToolMessage | `pre_model_hook` **+** `post_model_hook` |

**현재 우리는 두 전략 다 켜두었으므로 반드시 둘 다 붙여야 함.** `pre_model_hook`만 붙이면 요약이 반쪽짜리.

### 🚨 **Latency 주의 — 배포 후 즉시 모니터링 필수**

LangGraph는 기본적으로 **모든 super-step(노드 실행) 후 체크포인트 저장**.

공식 문서 사례:
- 6-노드 대화형 워크플로우(tool call 포함) = **62 API calls** (49 createEvent + 13 listEvents)
- 유저 요청당 **약 8.7초 추가 지연**

현재 상태:
- 긴 작업(멀티 툴 호출) 기준 이미 **약 70초** 걸리는 상황
- LTM 붙이면 위에 겹쳐 체크포인트 저장 호출 증가 → 단순 합산 시 80초 이상 될 가능성

**Phase 3 배포 즉시 모니터링:**
- 배포 전 baseline 측정 (현재 메시지별 평균 응답 시간 기록)
- 배포 직후 같은 시나리오 측정 → 증분 ≤ 3초 선에서 허용. 그 이상이면 체크포인트 빈도/범위 축소 검토
- CloudWatch Lambda Duration alarm 임계값(현재 p95 > 100s)과 충돌하지 않는지 확인

**완화 옵션 (필요 시):**
- LangGraph `checkpoint_during=False` 또는 선별적 노드만 체크포인트
- `pre_model_hook` 내부에서 `store.put`을 비동기(백그라운드 task)로 돌려 응답 경로에서 제외
- `listEvents` 호출 최소화 — 필요한 시점에만 조회
...
- Latency 회귀는 실사용자에게 직격타 — 배포 직후 48시간 관찰 필수
